...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const TurnUsageRow = react.memo(function TurnUsageRow(props) {
      ...
      // 真实模型：由 request/header Definition 发布在轮次数据里（节点不投影模型）
      const modelInfo = turnLoc !== null && turnLoc.data !== void 0 && typeof turnLoc.data.get === "function"
        ? turnLoc.data.get("turn-usage-model") : void 0;
      const effectiveModel = modelInfo !== null && typeof modelInfo === "object" && typeof modelInfo.model === "string"
        ? modelInfo.model : fold.model;
    ...
    function PriceSettingsRow() {
      ...
      /** Write a table to localStorage + textarea, close nothing, report status. */
      const commitTable = (table, msg) => {
        const json = JSON.stringify(table, null, 2);
        window.localStorage.setItem(PRICE_STORAGE_KEY, json);
        setText(json);
                  ...
                  children: [
                    ...
                    react_jsx_runtime.jsx("select", {
                      ...
                      onChange: (event) => {
                        ...
                        setModel(next);
                        try {
                          if (next === "") window.localStorage.removeItem(MODEL_OVERRIDE_KEY);
                          else window.localStorage.setItem(MODEL_OVERRIDE_KEY, next);
                        } catch (_e) { /* ignore */ }
                        setStatus(null);
                      },
                      children: [
    ...
    // ------------------------------------------------------- model definition
    /**
     * Conversation Definition that captures `request/header` events and
     * publishes the model in force per turn under the location key
     * "turn-usage-model". The client conversation nodes do NOT project the
     * model, but request/header events carry `header.config` (provider/model),
     * so this lets every turn row show and price by its real model.
     */
