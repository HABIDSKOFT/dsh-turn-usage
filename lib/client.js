...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * nodes, so pricing cannot derive it per call; the per-node field is used
     * when present, and finally "*" (see README). No user override: pricing
     * matches the price table by the turn's real model name.
     */
    ...
    const TurnUsageRow = react.memo(function TurnUsageRow(props) {
      ...
      const effectiveModel = (() => {
        ...
        return typeof fold.model === "string" && fold.model !== "" ? fold.model : null;
      })();
          ...
          react_jsx_runtime.jsx("div", {
            ...
            children: react_jsx_runtime.jsxs("div", {
              ...
              children: [
                ...
                react_jsx_runtime.jsx("div", {
                  style: descStyle,
                  children: "每百万 token 价格（人民币 ¥）。可用「添加模型/修改模型」按钮操作，或直接编辑 JSON：模型名 -> { input: 未命中输入, cacheRead: 缓存命中, cacheWrite: 写缓存, output: 输出 }。需要峰谷自动切换时用 { switchAt, peak, offPeak } 结构（北京时间高峰 09:00-12:00、14:00-18:00，空闲为高峰一半）。价格按每轮真实模型名匹配，未配置的模型按 * 兜底价。窗口高度可拖动，会自动记住。"
                }),
